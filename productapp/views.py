    ...
    return JsonResponse(
        {
            ...
            else None,  # Include previous_page_number
        }
    )
...
def liked_products(request):
    if request.user.is_authenticated:
        liked_product_ids = Like.objects.filter(user=request.user).values_list(
...
def details(request, product_id):
    ...
    liked_products = Like.objects.filter(user=request.user).values_list(
        "product", flat=True
    )
...
def add_to_cart(request):
    if request.method == "POST":
        ...
        cart_item, created = Cart.objects.get_or_create(
            user=request.user, product=shop_item
        )
...
def purchase(request, selection_id):
    ...
    if request.method == "POST":
        ...
        client = razorpay.Client(
            auth=("rzp_test_dTWp25pBQ5jW81", "Sg6ymJfWNf4atGBsqXhuaALE")
        )
        ...
        design = OrderDesign(
            user=request.user,
            info=user_profile,
            design_name=selection.product.title,
            amount=amount_str,
            designcolor=selection.selected_color,
            designsize=selection.selected_size,
    ...
    return render(
        request, "purchase.html", {"selection": selection, "user_profile": user_profile}
    )
...
def userorders(request):
    ...
    all_orders = sorted(
        chain(shop_orders, design_orders), key=attrgetter("created_at"), reverse=True
    )
...
def dashboard(request):
    ...
    cart_revenue = (
        OrderCart.objects.filter(paid=True).aggregate(
            total=Sum(F("amount"), output_field=FloatField())
        )["total"]
        or 0
    )
    design_revenue = (
        OrderDesign.objects.filter(paid=True).aggregate(
            total=Sum(F("amount"), output_field=FloatField())
        )["total"]
        or 0
    )
...
def upload_shop(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            shop_instance.cat = request.POST.get(
                "cat", ""
            )  # Get the 'cat' value from POST data
            shop_instance.save()
...
def uploadproduct(request):
    if request.method == "POST":
        try:
            ...
            if not all([image, title, product_id, tag, price, color, size]):
                return render(
                    request, "uploadproducts.html", {"error": "All fields are required"}
                )
            ...
            return render(request, "uploadproducts.html", {"error": str(e)})
...
def admin_order_detail(request, order_id, order_type):
    ...
    if request.method == "POST":
        ...
        order.save()
        return redirect(
            "productapp:admin_order_detail", order_id=order.id, order_type=order_type
